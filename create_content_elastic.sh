...
if [ "$version" == "7" ]; then
    ...
    for TEMPLATE_PATH in $(ls elastic-index-templates/tpl_7x/*jsonc); do
        ...
        curl $CURL_OPTIONS $CURL_CREDS -XPUT "$CURL_SCHEME://$IP_ELASTIC:9200/_template/$TEMPLATE_NAME?include_type_name=true" -H "Content-Type: application/json" --data-binary @$TEMPLATE_PATH
        echo -e ""
    done
